 ...
 * @author yccheok
 */
public abstract class AbstractSinaStockServer implements StockServer {
    protected abstract String getSinaCSVBasedURL();

    public AbstractSinaStockServer() {
    }
    ...
    private List<Stock> _getStocks(List<Code> codes) throws StockNotFoundException {
        ...
        System.out.println("AbstractSinaStockServer::_getStock(List<Code>)");
        if (codes.isEmpty()) {
            return stocks;
        }
        ...
        for (int i = 0; i < time; i++) {
            ...
            final StringBuilder stringBuilder = new StringBuilder(getSinaCSVBasedURL());
            ...
            for (int retry = 0; retry < NUM_OF_RETRY; retry++) {
                ...
                final List<Stock> tmpStocks = SinaStockFormat.getInstance().parse(respond);
        ...
        for (int i = start; i < endLoop; i++) {
            ...
            try {
                ...
                //for sina stock format
                //codeString = "sh" + codeString.replace(".SS", "").trim();
            } catch (UnsupportedEncodingException ex) {
       ...
       // codeString = "sh" + codeString.replace(".SS", "").trim();
        System.out.println("AbstractSinaStockServer::_getStock(List<Code>)codeString: " + codeString);
        ...
        for (int retry = 0; retry < NUM_OF_RETRY; retry++) {
            final String respond = org.yccheok.jstock.gui.Utils.getResponseBodyAsStringBasedOnProxyAuthOption(location);
            if (respond == null || respond.contains("FAILED")) {
                System.out.println("AbstractSinaStockServer::_getStock(List<Code>): Failed to get stock <" + _code + ">" + "info!");
                continue;
            }
            final List<Stock> tmpStocks = SinaStockFormat.getInstance().parse(respond);
        ...
        System.out.println("AbstractSinaStockServer::_getStock(List<Code>) End"); 
        return stocks;
    }
    ...
    private Stock _getStock(Code code) throws StockNotFoundException {
        ...
        System.out.println("AbstractSinaStockServer::_getStock(Code)");
        final String _code;
        try {
        ...
        for (int retry = 0; retry < NUM_OF_RETRY; retry++) {
            ...
            final List<Stock> stocks = SinaStockFormat.getInstance().parse(respond);
    ...
    // Sina server limit is 200. We shorter, to avoid URL from being too long.
    // Sina sometimes does complain URL for being too long.
